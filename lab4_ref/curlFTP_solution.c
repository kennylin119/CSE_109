...
/* having the / on the end results in directory listing */
...
static int ftpDownload(char *userid, char *password, char *server, char *pathname) {
  ...
  if (cp) {                                                    /* check if curl handle creation was successful, and if so, proceed */
    ...
    curl_easy_setopt(cp, CURLOPT_WRITEFUNCTION, ftpWrite);     /* declare our callback to be called when there's data to be written */
